...
void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Test(void)
{
  ...
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN, LOW);
  unsigned int return_signal = pulseIn(ECHO_PIN, HIGH);
  Serial.println(return_signal);
  ...
  //Serial.println(tempda); // Convert ping time to distance and print result (0 = outside set distance range, no ping echo)
  //Serial.println("cm");
  delay(250);
}
